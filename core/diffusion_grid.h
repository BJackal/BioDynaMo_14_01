 ...
 public:
  ...
  void ParametersCheck() {
    ...
    if ((1.0 * (1 - dc_[0]) * dt_) / (1.0 * box_length_ * box_length_) >=
      ...
      Log::Fatal(
          "DiffusionGrid",
          "The specified parameters of the diffusion grid for [",
          substance_name_,
...
#pragma omp parallel for collapse(2)
  ...
  void IncreaseConcentrationBy(size_t idx, double amount) {
    ...
    if (c1_[idx] < concentration_lower_threshold_) {
      c1_[idx] = concentration_lower_threshold_;
    }
  }
  ...
  void SetConcentrationLowerThreshold(double l) {
    concentration_lower_threshold_ = l;
  }

  double GetConcentrationLowerThreshold() {
    return concentration_lower_threshold_;
  }
 ...
 private:
  ...
  double concentration_lower_threshold_ = 0.0;
