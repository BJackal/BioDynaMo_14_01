...
// THERMAL GRID START , This makes sure any created thermal grid is in line with diffusion grids that will be made.

  void AddThermalGrid(ThermalGrid* tgrid) {
        uint64_t substance_id = tgrid->GetSubstanceId();
    auto search = thermal_grids_.find(substance_id);
    if (search != thermal_grids_.end()) {
      Log::Fatal("ResourceManager::AddThermalGrid",
                 "You tried to add a thermal grid with an already existing "
                 "substance id. Please choose a different substance id.");
    } else {
      thermal_grids_[substance_id] = tgrid;
    }
  }

  void RemoveThermalGrid(size_t substance_id) {
        auto search = thermal_grids_.find(substance_id);
    if (search != thermal_grids_.end()) {
      delete search->second;
      thermal_grids_.erase(search);
    } else {
      Log::Fatal("ResourceManager::AddThermalGrid",
                 "You tried to remove a thermal grid that does not exist.");
    }
  }

  ThermalGrid* GetThermalGrid(size_t substance_id) const{
  assert(substance_id < thermal_grids_.size() &&
           "You tried to access a thermal grid that does not exist!");
    return thermal_grids_.at(substance_id);
  }

  // TODO [Jack] It is unknown if this will be necessary outside of a catch all where someone trys to call Temperature.

    ThermalGrid* GetThermalGrid(std::string substance_name) const {
    for (auto& el : thermal_grids_) {
      auto& dg = el.second;
      if (dg->GetSubstanceName() == substance_name) {
        return dg;
      }
  ...
  // TODO [Jack] Instead of applying on all Thermal grids it would be better to inform the user that only one thermal Grid exists, again here as a catch right now.
    template <typename TFunctor>
  void ApplyOnAllThermalGrids(TFunctor&& f) const {
    for (auto& el : thermal_grids_) {
      f(el.second);
    }
  }
...
//THERMAL GRID END
 ...
 protected:
  ...
  /// Maps a thermal grid ID same as diffusion
  std::unordered_map<uint64_t, ThermalGrid*> thermal_grids_;
