...
class CatalystAdaptor {
 public:
  ...
  ~CatalystAdaptor() {
    ...
    if (param->export_visualization_ &&
      ...
      GenerateSimulationInfoJson(vtk_so_grids_, vtk_dgrids_, vtk_tgrids_);
 ...
 private:
  ...
  std::unordered_map<std::string, VtkThermalGrid*> vtk_tgrids_;
  ...
  void Initialize() {
    ...
    if (param->live_visualization_ || param->export_visualization_) {
      ...
      for (auto& entry : param->visualize_thermal_) {
        vtk_tgrids_[entry.nameT_] =
            new VtkThermalGrid(entry.nameT_, data_description);
      }
    }
  }
  ...
  void CreateVtkObjects(
    ...
    BuildThermalGridVTKStructures(data_description);
  }
  ...
  void ProcessThermalGrid(
      const ThermalGrid* tgrid,
      const vtkNew<vtkCPDataDescription>& data_description) {
    auto* param = Simulation::GetActive()->GetParam();
    auto name = tgrid->GetSubstanceName();

    // get visualization config
    const Param::VisualizeThermal* vt = nullptr;
    for (auto& entry : param->visualize_thermal_) {
      if (entry.nameT_ == name) {
        vt = &entry;
      }
    }

    if (vt != nullptr) {
      auto* vtg = vtk_tgrids_[tgrid->GetSubstanceName()];
      if (!vtg->used_) {
        vtg->Init();
      }
      ...
      if (exclusive_export_viz_ ||
          (g_processor_->RequestDataDescription(
              data_description.GetPointer())) != 0) {
        auto num_boxes = tgrid->GetNumBoxesArray();
        auto tgrid_dimensions = tgrid->GetDimensions();
        auto box_length = tgrid->GetBoxLength();
        auto total_boxes = tgrid->GetNumBoxes();

        double origin_x = tgrid_dimensions[0];
        double origin_y = tgrid_dimensions[2];
        double origin_z = tgrid_dimensions[4];
        vtg->data_->SetOrigin(origin_x, origin_y, origin_z);
        vtg->data_->SetDimensions(num_boxes[0], num_boxes[1], num_boxes[2]);
        vtg->data_->SetSpacing(box_length, box_length, box_length);

        if (vtg->temperature_) {
          auto* co_ptr = const_cast<double*>(tgrid->GetAllConcentrations());
          vtg->temperature_->SetArray(co_ptr,
                                        static_cast<vtkIdType>(total_boxes), 1);
        }
        if (vtg->gradientT_) {
          auto gr_ptr = const_cast<double*>(tgrid->GetAllGradients());
          vtg->gradientT_->SetArray(gr_ptr,
                                   static_cast<vtkIdType>(total_boxes * 3), 1);
        }
      }
    }
  ...
  /// Create the required vtk objects to visualize thermal grid.
  void BuildThermalGridVTKStructures(
      const vtkNew<vtkCPDataDescription>& data_description) {
    auto* rm = Simulation::GetActive()->GetResourceManager();

    rm->ApplyOnAllThermalGrids([&](ThermalGrid* tgrid) {
      ProcessThermalGrid(tgrid, data_description);
    });
  }
  ...
  void WriteToFile(size_t step) {
    ...
    for (auto& entry : vtk_dgrids_) {
      ...
      dgrid_writer->Update();
        ...
        for (auto& entry : vtk_tgrids_) {
      vtkNew<vtkXMLPImageDataWriter> dgrid_writer;

      const auto& substance_name = entry.second->nameT_;
      auto filename =
          Concat(sim->GetOutputDir(), "/", substance_name, "-", step, ".pvti");
      dgrid_writer->SetFileName(filename.c_str());
      dgrid_writer->SetInputData(entry.second->data_);
  ...
  static void GenerateSimulationInfoJson(
      ...
      const std::unordered_map<std::string, VtkThermalGrid*>& vtk_tgrids) {
    ...
    for (uint64_t i = 0; i < num_substances; i++) {
      ...
      auto& nameT =  param->visualize_thermal_[i].nameT_;
      auto search = vtk_dgrids.find(name);
      auto searchT = vtk_tgrids.find(nameT);

      if (search == vtk_dgrids.end()) {
        continue;
      }
      if (searchT == vtk_tgrids.end()) {
        continue;
      }
