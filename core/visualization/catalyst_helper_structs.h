...
struct VtkThermalGrid {
  VtkThermalGrid(const std::string& nameT,
                   const vtkNew<vtkCPDataDescription>& data_description) {
    data_ = vtkImageData::New();
    nameT_ = nameT;
    ...
    if (vt->temperature_) {
      ...
      temperature->SetName("Substance Concentration");
      temperature_ = temperature.GetPointer();
      data_->GetPointData()->AddArray(temperature.GetPointer());
    }
    if (vt->gradientT_) {
      vtkNew<vtkDoubleArray> gradientT;
      gradientT->SetName("Diffusion Gradient");
      gradientT->SetNumberOfComponents(3);
      gradientT_ = gradientT.GetPointer();
      data_->GetPointData()->AddArray(gradientT.GetPointer());
    }

    data_description->AddInput(nameT.c_str());
    data_description->GetInputDescriptionByName(nameT.c_str())->SetGrid(data_);
  }
  ...
  std::string nameT_;
  vtkImageData* data_ = nullptr;
  vtkDoubleArray* temperature_ = nullptr;
  vtkDoubleArray* gradientT_ = nullptr;
};
// TODO
