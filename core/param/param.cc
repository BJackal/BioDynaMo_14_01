...
void Param::AssignFromConfig(const std::shared_ptr<cpptoml::table>& config) {
  ...
  //   visualize_thermal_ TODO
  auto visualize_thermal_tarr =
      config->get_table_array("visualize_thermal");
  if (visualize_thermal_tarr) {
    for (const auto& table : *visualize_thermal_tarr) {
      if (table->contains("name")) {
        auto name = table->get_as<std::string>("name");
        if (!name) {
          Log::Warning("AssignFromConfig",
        ...
        if (table->contains("concentration")) {
          auto concentration = table->get_as<bool>("concentration");
          if (concentration) {
            td.temperature_ = *concentration;
          }
        }
        if (table->contains("gradient")) {
          auto gradient = table->get_as<bool>("gradient");
          if (gradient) {
            td.gradientT_ = *gradient;
          }
        }

        visualize_thermal_.push_back(td);
      }
    }
  }
  // TODO
